			...
			await streamChat(
				...
				(e) => {
					if (e.type === 'token') {
						...
						toolHint =
							e.name === 'get_findings_for_combo' ? 'Looking up drivers…' : 'Fetching combos…';
					} else if (e.type === 'done') {
						...
						<div class="chat-head-actions">
							...
							>
								<svg viewBox="0 0 24 24" width="15" height="15" fill="none" aria-hidden="true">
									<path
										d="M4 20 h4 L19 9 l-4 -4 L4 16 v4 Z M14 6 l4 4"
										stroke="currentColor"
										stroke-width="1.6"
							...
							>
								<svg viewBox="0 0 24 24" width="15" height="15" fill="none" aria-hidden="true">
									{#if expanded}
										<path
											d="M10 10 L4 4 M4 8 V4 H8 M14 10 L20 4 M20 8 V4 H16 M10 14 L4 20 M4 16 V20 H8 M14 14 L20 20 M20 16 V20 H16"
											stroke="currentColor"
											stroke-width="1.6"
											stroke-linecap="round"
											stroke-linejoin="round"
							...
							<button
								...
								title="Chat history">History</button
							>
						...
						<div class="chat-history">
							...
							{:else}
								{#each conversations as c (c.id)}
									<button
										class="history-item"
										class:current={c.id === currentConvId}
										onclick={() => switchTo(c.id)}
									>
										<span class="history-title">{c.title ?? 'New chat'}</span>
										<span class="history-meta"
											>{fmtConvDate(c.created_at)} · {c.message_count} msg{c.message_count === 1
												? ''
												: 's'}</span
										>
									</button>
		...
		<button class="chat-fab" onclick={toggle} aria-label="Ask Cortex" title="Ask Cortex">
			<svg viewBox="0 0 24 24" width="34" height="34" fill="#fff" aria-hidden="true">
				<g transform="translate(0 2.2)">
					...
					<path
						d="M18 3 L18.7 4.8 L20.5 5.5 L18.7 6.2 L18 8 L17.3 6.2 L15.5 5.5 L17.3 4.8 Z"
						opacity="0.9"
					/>
				</g>
			</svg>
...
<style>
	...
	.prose :global(th) {
		font-weight: 600;
		background: color-mix(in srgb, var(--color-page-fg) 8%, transparent);
	}
	.tool-hint {
		font-size: 0.75rem;
		color: var(--color-page-fg-muted);
	...
	.sel-toggle input:checked + .switch-track {
		background: var(--color-accent);
	}
	.sel-toggle input:checked + .switch-track .switch-thumb {
		left: calc(100% - 0.95rem);
	}
	.sel-toggle input:focus-visible + .switch-track {
		outline: 2px solid var(--color-accent);
		outline-offset: 2px;
	}
</style>
