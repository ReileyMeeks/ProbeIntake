<script lang="ts">
    ...
    */
	...
	// Rebuilt in a later task once the real Probe Intake routes/nav exist.
	const navItems = $derived.by(() => {
		return [] as Array<{ href: string; label: string; icon: string }>;
	});
...
{:else}
	...
	<button
		type="button"
		class="mobile-hamburger glass glass-press focus-glass"
	...
	{#if mobileMenuOpen}
		...
		>
			<a
				href="/"
				class="brand brand-stack brand-link focus-glass"
			...
			{#if auth.state.isAuthenticated && auth.state.user}
				<div class="user-chip" title={auth.state.user.email ?? ''}>
					<span class="user-name"
						>{auth.state.user.name ?? auth.state.user.email ?? 'Signed in'}</span
					>
	...
	<div class="app-shell" data-shell={shell}>
		{#if shell === 'vertical'}
			...
			>
				...
				<div class="sidebar-footer">
					<GlassButton size="sm" onclick={onToggleShell} title="Hide sidebar" class="footer-action">
...
<style>
	...
	.sidebar-footer :global(.footer-action) {
		flex: 1;
	}
